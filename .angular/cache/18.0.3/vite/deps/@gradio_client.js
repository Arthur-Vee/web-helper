...
function get_jwt(space, token, cookies) {
  return __async(this, null, function* () {
    try {
      const r = yield fetch(`https://huggingface.co/api/spaces/${space}/jwt`, {
        headers: __spreadValues({
          Authorization: `Bearer ${token}`
        }, cookies ? { Cookie: cookies } : {})
      });
      const jwt = (yield r.json()).token;
      return jwt || false;
    } catch (e) {
      return false;
    }
...
function resolve_config(endpoint) {
  return __async(this, null, function* () {
    var _a;
    ...
    if (typeof window !== "undefined" && window.gradio_config && location.origin !== "http://localhost:9876" && !window.gradio_config.dev_mode) {
      ...
      const response = yield this.fetch(config_url, {
        headers,
        credentials: "include"
      });
      ...
      if ((response == null ? void 0 : response.status) === 200) {
        let config = yield response.json();
...
function resolve_cookies() {
  return __async(this, null, function* () {
    const { http_protocol, host } = yield process_endpoint(
      this.app_reference,
      this.options.hf_token
    );
    try {
      if (this.options.auth) {
        const cookie_header = yield get_cookie_header(
          http_protocol,
          host,
          this.options.auth,
          this.fetch,
...
function get_cookie_header(http_protocol, host, auth, _fetch, hf_token) {
  return __async(this, null, function* () {
    ...
    const res = yield _fetch(`${http_protocol}//${host}/${LOGIN_URL}`, {
      headers,
      method: "POST",
      body: formData,
      credentials: "include"
...
function process_endpoint(app_reference, hf_token) {
  return __async(this, null, function* () {
    const headers = {};
    if (hf_token) {
      headers.Authorization = `Bearer ${hf_token}`;
    ...
    if (RE_SPACE_NAME.test(_app_reference)) {
      try {
        ...
        const _host = (yield res.json()).host;
    ...
    if (category === "named_endpoints" || category === "unnamed_endpoints") {
      ...
      Object.entries(api_info[category]).forEach(
        ([endpoint, { parameters, returns }]) => {
          ...
          const transform_type = (data, component, serializer, signature_type) => __spreadProps(__spreadValues({}, data), {
...
var map_data_to_params = (data, api_info) => {
  ...
  return __async(this, null, function* () {
    ...
    try {
      ...
      if (semiver((config == null ? void 0 : config.version) || "2.0.0", "3.30") < 0) {
        response = yield this.fetch(SPACE_FETCHER_URL, {
          method: "POST",
          body: JSON.stringify({
            serialize: false,
      ...
      let api_info = yield response.json();
      if ("api" in api_info) {
...
function upload_files(root_url, files, upload_id) {
  return __async(this, null, function* () {
    var _a;
    const headers = {};
    ...
    for (let i = 0; i < files.length; i += chunkSize) {
      ...
      try {
        ...
        response = yield this.fetch(upload_url, {
          method: "POST",
          body: formData,
          headers,
          credentials: "include"
      ...
      if (!response.ok) {
        const error_text = yield response.text();
...
function upload(file_data, root_url, upload_id, max_file_size) {
  return __async(this, null, function* () {
    ...
    return yield Promise.all(
      yield this.upload_files(root_url, files, upload_id).then(
          ...
          if (response.error) {
            ...
            if (response.files) {
              return response.files.map((f, i) => {
                const file = new FileData(__spreadProps(__spreadValues({}, file_data[i]), {
                  path: f,
                  url: root_url + "/file=" + f
                }));
                return file;
...
function prepare_files(files, is_stream) {
  return __async(this, null, function* () {
    return files.map(
      (f) => new FileData({
        path: f.name,
...
function walk_and_store_blobs(_0) {
  return __async(this, arguments, function* (data, type = void 0, path = [], root = false, endpoint_info = void 0) {
    if (Array.isArray(data)) {
      let blob_refs = [];
      yield Promise.all(
        data.map((_, index) => __async(this, null, function* () {
          var _a;
          let new_path = path.slice();
          new_path.push(String(index));
          const array_refs = yield walk_and_store_blobs(
            data[index],
...
function handle_blob(endpoint, data, api_info) {
  return __async(this, null, function* () {
    const self = this;
    yield process_local_file_commands(self, data);
    const blobRefs = yield walk_and_store_blobs(
      data,
      void 0,
      [],
      true,
    ...
    const results = yield Promise.all(
      blobRefs.map((_0) => __async(this, [_0], function* ({ path, blob, type }) {
        if (!blob)
          return { path, type };
        const response = yield self.upload_files(endpoint, [blob]);
...
function process_local_file_commands(client2, data) {
  return __async(this, null, function* () {
    var _a, _b;
    ...
    yield recursively_process_commands(client2, data);
  });
}
function recursively_process_commands(_0, _1) {
  return __async(this, arguments, function* (client2, data, path = []) {
    for (const key in data) {
      if (data[key] instanceof Command) {
        yield process_single_command(client2, data, key);
      } else if (typeof data[key] === "object" && data[key] !== null) {
        yield recursively_process_commands(client2, data[key], [...path, key]);
      }
    }
  });
}
function process_single_command(client2, data, key) {
  return __async(this, null, function* () {
    var _a, _b;
    ...
    try {
      ...
      if (typeof process !== "undefined" && process.versions && process.versions.node) {
        const fs = yield import("./promises-WIPB4ZIS.js");
        const path = yield import("./path-IQS5KXZN.js");
        fullPath = path.resolve(process.cwd(), cmd_item.meta.path);
        fileBuffer = yield fs.readFile(fullPath);
      } else {
        throw new Error(NODEJS_FS_ERROR_MSG);
      }
      const file = new Blob([fileBuffer], { type: "application/octet-stream" });
      const response = yield client2.upload_files(root, [file]);
      const file_url = response.files && response.files[0];
      if (file_url) {
...
function post_data(url, body, additional_headers) {
  return __async(this, null, function* () {
    const headers = { "Content-Type": "application/json" };
    ...
    try {
      var response = yield this.fetch(url, {
        method: "POST",
        body: JSON.stringify(body),
        headers: __spreadValues(__spreadValues({}, headers), additional_headers),
        credentials: "include"
      });
    } catch (e) {
...
function predict(endpoint, data) {
  return __async(this, null, function* () {
    ...
    return new Promise((resolve, reject) => __async(this, null, function* () {
      ...
      let result;
      try {
        for (var iter = __forAwait(app), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
...
function check_space_status(id, type, status_callback) {
  return __async(this, null, function* () {
    ...
    switch (stage) {
      ...
      case "PAUSED":
        status_callback({
          ...
          detail: stage,
          discussions_enabled: yield discussions_enabled(space_name)
        });
        break;
      case "RUNNING":
...
function discussions_enabled(space_id) {
  return __async(this, null, function* () {
    try {
      const r = yield fetch(
...
function get_space_hardware(space_id, hf_token) {
  return __async(this, null, function* () {
    const headers = {};
    if (hf_token) {
      headers.Authorization = `Bearer ${hf_token}`;
...
function set_space_timeout(space_id, timeout, hf_token) {
  return __async(this, null, function* () {
    const headers = {};
    if (hf_token) {
      headers.Authorization = `Bearer ${hf_token}`;
...
function duplicate(app_reference, options) {
  return __async(this, null, function* () {
    ...
    const user = (yield (yield fetch(`https://huggingface.co/api/whoami-v2`, {
      headers
    })).json()).name;
    ...
    try {
      if (!hardware) {
        original_hardware = yield get_space_hardware(app_reference, hf_token);
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.status === 409) {
        try {
          const client2 = yield Client.connect(`${user}/${space_name}`, options);
          return client2;
        } catch (error) {
      ...
      const duplicated_space = yield response.json();
      yield set_space_timeout(`${user}/${space_name}`, timeout || 300, hf_token);
...
function events(res, signal) {
  return __asyncGenerator(this, null, function* () {
    ...
    for (; ; ) {
      ...
      line = yield new __await(reader.read());
      if (line.done)
        return;
      if (!line.value) {
        if (event)
...
function open_stream() {
  return __async(this, null, function* () {
    ...
    stream2.onmessage = function(event) {
      return __async(this, null, function* () {
...
function readable_stream(input, init = {}) {
  ...
  stream(input, init).then((res) => __async(this, null, function* () {
    instance.readyState = instance.OPEN;
    try {
      try {
        for (var iter = __forAwait(res), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
  ...
  try {
    ...
    function cancel() {
      return __async(this, null, function* () {
        ...
        complete = _status;
        fire_event(__spreadProps(__spreadValues({}, _status), {
          type: "status",
          endpoint: _endpoint,
          fn_index
        }));
        ...
        try {
          ...
          if ("event_id" in cancel_request) {
            yield fetch2(`${config.root}/cancel`, {
              headers: { "Content-Type": "application/json" },
              method: "POST",
              body: JSON.stringify(cancel_request)
            });
          }
          yield fetch2(`${config.root}/reset`, {
            headers: { "Content-Type": "application/json" },
            method: "POST",
    ...
    const resolve_heartbeat = (config2) => __async(this, null, function* () {
      yield this._resolve_hearbeat(config2);
    });
    function handle_render_config(render_config) {
      return __async(this, null, function* () {
        if (!config)
          return;
        ...
        yield resolve_heartbeat(config);
        fire_event({
          type: "render",
          data: render_config,
          endpoint: _endpoint,
    ...
    this.handle_blob(config.root, resolved_data, endpoint_info).then(
      (_payload) => __async(this, null, function* () {
        var _a2;
        ...
        if (skip_queue(fn_index, config)) {
          ...
          const { ws_protocol, host } = yield process_endpoint(
            app_reference,
            hf_token
          );
          fire_event({
          ...
          websocket.onmessage = function(event) {
            ...
            if (type === "update" && status && !complete) {
              ...
              websocket.send(JSON.stringify(__spreadProps(__spreadValues({}, payload), { session_hash })));
              ...
              fire_event(__spreadProps(__spreadValues({
                type: "status",
                time: /* @__PURE__ */ new Date()
              }, status), {
          ...
          stream2.onmessage = function(event) {
            return __async(this, null, function* () {
              ...
              if (type === "update" && status && !complete) {
                ...
                let [_, status2] = yield post_data2(`${config.root}/queue/data`, __spreadProps(__spreadValues({}, payload), {
                  session_hash,
                  event_id
                }));
                if (status2 !== 200) {
          ...
          post_data_promise.then((_0) => __async(this, [_0], function* ([response, status]) {
            if (status === 503) {
              fire_event({
                type: "status",
                stage: "error",
                  ...
                  try {
                    ...
                    if (data2) {
                      ...
                      if (data2.render_config) {
                        yield handle_render_config(data2.render_config);
                      }
                      if (complete) {
                        fire_event(__spreadProps(__spreadValues({
                          type: "status",
    ...
    const iterator = {
      ...
      next,
      throw: (value) => __async(this, null, function* () {
        push_error(value);
        return next();
      }),
      return: () => __async(this, null, function* () {
...
var Client = class {
  ...
  init() {
    return __async(this, null, function* () {
      ...
      if ((typeof window === "undefined" || !("WebSocket" in window)) && !global.WebSocket) {
        const ws = yield import("./wrapper-CviSselG-MSNH56XD.js");
        global.WebSocket = ws.WebSocket;
      }
      try {
        if (this.options.auth) {
          yield this.resolve_cookies();
        }
        yield this._resolve_config().then(
          ({ config }) => this._resolve_hearbeat(config)
        );
      } catch (e) {
        throw Error(e);
      }
      this.api_info = yield this.view_api();
      this.api_map = map_names_to_ids(((_a = this.config) == null ? void 0 : _a.dependencies) || []);
    });
  }
  _resolve_hearbeat(_config) {
    return __async(this, null, function* () {
      if (_config) {
        this.config = _config;
        if (this.config && this.config.connect_heartbeat) {
          if (this.config.space_id && this.options.hf_token) {
            this.jwt = yield get_jwt(
      ...
      if (_config.space_id && this.options.hf_token) {
        this.jwt = yield get_jwt(_config.space_id, this.options.hf_token);
      }
      if (this.config && this.config.connect_heartbeat) {
  ...
  static connect(_0) {
    return __async(this, arguments, function* (app_reference, options = {
      events: ["data"]
    }) {
  ...
  _resolve_config() {
    return __async(this, null, function* () {
      const { http_protocol, host, space_id } = yield process_endpoint(
        this.app_reference,
        this.options.hf_token
      );
      ...
      let config;
      try {
        config = yield this.resolve_config(`${http_protocol}//${host}`);
        if (!config) {
          throw new Error(CONFIG_ERROR_MSG);
        }
  ...
  handle_space_success(status) {
    return __async(this, null, function* () {
      ...
      if (status.status === "running") {
        try {
          this.config = yield this._resolve_config();
          if (!this.config) {
            throw new Error(CONFIG_ERROR_MSG);
          }
          const _config = yield this.config_success(this.config);
          return _config;
        } catch (e) {
          if (status_callback) {
            status_callback({
  ...
  component_server(component_id, fn_name, data) {
    return __async(this, null, function* () {
      var _a;
      if (!this.config) {
        throw new Error(CONFIG_ERROR_MSG);
      ...
      try {
        const response = yield this.fetch(`${root_url}/component_server/`, {
          method: "POST",
          body,
          headers,
          credentials: "include"
...
function client(_0) {
  return __async(this, arguments, function* (app_reference, options = {
    ...
    return yield Client.connect(app_reference, options);
  });
}
function duplicate_space(app_reference, options) {
  return __async(this, null, function* () {
    return yield Client.duplicate(app_reference, options);
  });
}
export {
  Client,
